...
class CmandrillMessages
{
	 ...
	 *                              - inline_css boolean whether or not to automatically inline all CSS styles provided in the message HTML - only for
	 ...
	 *                              - signing_domain string a custom domain to use for SPF/DKIM signing instead of mandrill (for "via" or "on behalf of"
	 *                                in email clients)
	 ...
	 *                                first 100 we see, so this should not be unique or change frequently.  Tags should be 50 characters or less.
	 ...
	 *                              - google_analytics_domains array an array of strings indicating for which any matching URLs will automatically have
	 *                                Google Analytics parameters appended to their query string automatically.
	 *                              - google_analytics_campaign array|string optional string indicating the value to set for the utm_campaign tracking
	 *                                parameter. If this isn't provided the email's from address will be used instead.
	 *                              - metadata array metadata an associative array of user metadata. Mandrill will store this metadata and make it
	 *                                available for retrieval. In addition, you can select up to 10 metadata fields to index and make searchable
	 ...
	 * @param   boolean  $async    - enable a background sending mode that is optimized for bulk sending. In async mode, messages/send will
	 ...
	 * @param   string   $ip_pool  - the name of the dedicated ip pool that should be used to send the message. If you do not have any dedicated IPs,
	 *                                this parameter has no effect. If you specify a pool that does not exist, your default pool will be used instead.
	 * @param   string   $send_at  - when this message should be sent as a UTC timestamp in YYYY-MM-DD HH:MM:SS format. If you specify a time in the
	...
	public function send($message, $async = false, $ip_pool = null, $send_at = null)
	{
		...
		return $this->master->call('messages/send', $params);
	}
	 ...
	 *                                       - track_opens boolean whether or not to turn on open tracking for the message
	 *                                       - track_clicks boolean whether or not to turn on click tracking for the message
	 *                                       - auto_text boolean whether or not to automatically generate a text part for messages that are not
	 *                                       given text
	 *                                       - auto_html boolean whether or not to automatically generate an HTML part for messages that are not given
	 *                                       HTML
	 *                                       - inline_css boolean whether or not to automatically inline all CSS styles provided in the message HTML -
	 ...
	 *                                       - bcc_address string an optional address to receive an exact copy of each recipient's email
	 *                                       - tracking_domain string a custom domain to use for tracking opens and clicks instead of mandrillapp.com
	 *                                       - signing_domain string a custom domain to use for SPF/DKIM signing instead of mandrill (for "via" or "on 
     *                                       behalf of" in email clients)
	 *                                       - return_path_domain string a custom domain to use for the messages's return-path
	 *                                       - merge boolean whether to evaluate merge tags in the message. Will automatically be set to true if either
	 ...
	 *                                       only store the first 100 we see, so this should not be unique or change frequently.  Tags should be 50
	 *                                       characters or less.  Any tags starting with an underscore are reserved for internal use and
	 *                                       will cause errors.
	 *                                       - tags[] string a single tag - must not start with an underscore
	 *                                       - subaccount string the unique id of a subaccount for this message - must already exist or will fail with
	 *                                       an error
	 ...
	 *                                       - google_analytics_campaign array|string optional string indicating the value to set for the utm_campaign
	 *                                       tracking parameter. If this isn't provided the email's from address will be used instead.
	 *                                       - metadata array metadata an associative array of user metadata. Mandrill will store this metadata and
	 *                                       make it available for retrieval. In addition, you can select up to 10 metadata fields to index and make
	 ...
	 *                                       - content string the content of the image as a base64-encoded string
	 * @param   boolean  $async             enable a background sending mode that is optimized for bulk sending. In async mode, messages/send will
	 *                                       immediately return a status of "queued" for every recipient. To handle rejections when sending in async
	 *                                       mode, set up a webhook for the 'reject' event. Defaults to false for messages with no more than
	 *                                       10 recipients; messages with more than 10 recipients are always sent asynchronously, regardless of the
	 *                                       value of async.
	 * @param   string   $ip_pool           the name of the dedicated ip pool that should be used to send the message. If you do not have any
	 *                                       dedicated IPs, this parameter has no effect. If you specify a pool that does not exist, your default pool
	 *                                       will be used instead.
	 * @param   string   $send_at           when this message should be sent as a UTC timestamp in YYYY-MM-DD HH:MM:SS format. If you specify a time
	...
	public function sendTemplate($template_name, $template_content, $message, $async = false, $ip_pool = null, $send_at = null)
	{
		...
		return $this->master->call('messages/send-template', $params);
	}
	...
	public function search($query = '*', $date_from = null, $date_to = null, $tags = null, $senders = null, $api_keys = null, $limit = 100)
	{
		...
		return $this->master->call('messages/search', $params);
	}
	...
	{
		$params = array("query" => $query, "date_from" => $date_from, "date_to" => $date_to, "tags" => $tags, "senders" => $senders);

		return $this->master->call('messages/search-time-series', $params);
	}
	...
	public function info($id)
	{
		...
		return $this->master->call('messages/info', $params);
	}
	...
	{
		$params = array("raw_message" => $raw_message);

		return $this->master->call('messages/parse', $params);
	}
	 ...
	 * @param   array|null   $to                  - optionally define the recipients to receive the message - otherwise we'll use the To, Cc, and Bcc
	 ...
	 *                                              messages/sendRaw will immediately return a status of "queued" for every recipient. To handle
	 *                                              rejections when sending in async mode, set up a webhook for the 'reject' event. Defaults to false
	 *                                              for messages with no more than 10 recipients; messages with more than 10 recipients are always sent
	 ...
	 * @param   string       $send_at             - when this message should be sent as a UTC timestamp in YYYY-MM-DD HH:MM:SS format. If you specify
	...
	public function sendRaw(
		$raw_message, $from_email = null, $from_name = null, $to = null, $async = false, $ip_pool = null, $send_at = null, $return_path_domain = null)
	{
		$params = array(
		...
		return $this->master->call('messages/send-raw', $params);
	}
	...
	public function listScheduled($to = null)
	{
		...
		return $this->master->call('messages/list-scheduled', $params);
	}
	...
	public function cancelScheduled($id)
	{
		...
		return $this->master->call('messages/cancel-scheduled', $params);
	}
	...
	{
		$params = array("id" => $id, "send_at" => $send_at);

		return $this->master->call('messages/reschedule', $params);
	}
}
