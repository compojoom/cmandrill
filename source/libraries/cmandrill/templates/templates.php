...
class CmandrillTemplates
{
	 ...
	 *     - publish_code string the full HTML code of the template, with mc:edit attributes marking the editable elements that are available as
	 *       published, if it has been published
	 ...
	 *     - published_at string the date and time the template was last published as a UTC string in YYYY-MM-DD HH:MM:SS format, or null if it has
	 *       not been published
	...
	public function info($name)
	{
		...
		return $this->master->call('templates/info', $params);
	}
	...
	public function update($name, $from_email = null, $from_name = null, $subject = null, $code = null, $text = null, $publish = true)
	{
		...
		return $this->master->call('templates/update', $params);
	}
	...
	public function publish($name)
	{
		...
		return $this->master->call('templates/publish', $params);
	}
	 ...
	 *         - publish_from_name string the default sender from name for the template, if provided
	 *         - publish_text string the default text part of messages sent with the template, if provided
	 *         - published_at string the date and time the template was last published as a UTC string in YYYY-MM-DD HH:MM:SS format, or null if it
	...
	public function getList()
	{
		...
		return $this->master->call('templates/list', $params);
	}
	...
	public function timeSeries($name)
	{
		...
		return $this->master->call('templates/time-series', $params);
	}
	...
	{
		$params = array("template_name" => $template_name, "template_content" => $template_content, "merge_vars" => $merge_vars);
